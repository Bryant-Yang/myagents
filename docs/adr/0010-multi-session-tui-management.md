...
- 主界面的固定状态首行始终显示当前 Host backend 与 worker 就绪比例；仅当其他
  会话存在运行中任务或未读终态时追加后台计数。该汇总只读取 SessionManager
  已加载 runtime 与 detached 状态，不为每秒刷新扫描磁盘。
...
- 权限决策器由共享 TUI 承载；默认弹窗明确显示项目、会话标题、agent 与工具，
  ADR-0016 显式危险模式则按稳定 room id 隔离自动决策；结果只回给原始
