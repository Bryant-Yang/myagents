...
- 模型可在唯一计划 JSON 外附带 fenced Markdown 或说明文字；解析器逐个解码完整
  JSON object，只接受唯一一个具备路由合同键的对象。多个路由/计划对象保持
  fail-closed，禁止用贪婪文本截取拼接相邻花括号。
...
2. 计划解析拒绝未知/未就绪 agent、1/5 步、空任务和单一参与者循环。
   同时接受说明文字中的无关 object 加唯一合法计划，并拒绝多个权威路由 object。
