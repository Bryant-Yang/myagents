...
一条 `/discuss` 是一个 CommandBus command，只有一条真实 user timeline 记录：
...
1. parser 覆盖合法命令、未知/重复 agent、主持人冲突、缺失主题以及轮数/人数
   上下界；非 `/discuss` 文本不误判。
2. fake agents 证明同轮并发、跨轮可见其他参与者回复、只写一条 user 记录、
   全部记录共享一个 `command_id`，且最后只有一次 moderator 仲裁。
3. 参与者失败后不进入下一轮，moderator 仍总结，CommandBus 最终标记 failed。
4. R5 gate 固定人数/轮数硬上限并拒绝 `_dispatch_discussion` 递归 dispatch；
   负向探针能失败，恢复后通过。
5. TUI `/discuss` 候选和精确命令帮助可用；带参数命令经同一 CommandBus，MCP
   无需增加新方法即可提交。
6. 授权的真实模型用例通过 MCP bridge 执行 Kimi/OpenCode 两轮与 host 仲裁，
...
用户可以一条命令安排指定成员完成可恢复、可观察、可取消的短讨论。最大模型调用
