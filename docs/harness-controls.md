...
| C17 | 运行中插话只走显式已验收 capability，且持久化先于协议写入 | ADR-0018；ADR-0009；SPEC UC-INTERJECT-001 | `test_m3_bus.py`、`test_tui_completion.py`、`test_phase2.py`、Pi client/adapter fake contract；R2/R4/R6 | 阻断；恢复唯一活动 delivery、能力检查、Pi commit gate、requested no-replay event 和不支持 transport 的 fail-closed 行为 | Bryant Yang |
