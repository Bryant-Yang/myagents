...
- **交互层级**：聊天主线只承载 user、agent/host 回复和必须独立可见的失败；
  ...
  面板失焦时只显示最多三张未结束卡和最近一张终态卡，运行/取消中优先于排队，
  并以一行计数说明其余任务已收起；默认折叠视口最多 7 行，避免历史任务挤占
  聊天区。`Ctrl+G` 聚焦后显示完整近期列表并恢复最多 14 行滚动视口，`Esc`
  或鼠标/键盘把焦点移出活动区时重新紧凑投影；最近终态以实际 terminal 迁移
  顺序为准，不按 command 创建顺序猜测。
...
- **高频分支**：adapter 继承初始 tool title/command 并压缩重复 update；
  ...
  汇总。非浏览态的卡片选择是确定性纯投影，不删除完整 feed 或持久事件；
  `Ctrl+G` 聚焦活动区并默认选中最近卡，`↑↓` 循环选择，`Enter` 逐卡
  展开脱敏明细，`Esc` 返回输入框；`/details` 切换当前选中卡，否则只切换
  最近一张卡。持久房间展开时按 command 异步加载执行日志，并以概览、阶段、
  工具/权限、插话/取消控制和输出统计呈现；partial 正文不重复显示，直接回答
  明确标注未调用工具或权限。重启后内存卡为空时，首次 `/details` 按需恢复最近
  一个持久任务。读取最多保留 200 条首尾代表事件，UI 最多展示 80 条并优先保留
  生命周期、工具、权限和控制证据，两层截断都显示省略量；完整事件统计不因采样
  失真。协议后续补发 tool ID 时迁移原逻辑项，
...
- **验收**：`events.read` 可分页读取；heartbeat 累计且界面不刷行；200 条相同
  ...
  十二张终态卡默认只占一张卡和一行收起提示，聚焦后十二张均可滚动到达；
  `/details` 只切换当前或最近卡；展开态可见持久过程概览、最新 heartbeat、
  工具终态、权限/控制事件、输出统计与脱敏命令，回答正文不重复，错误仍在
