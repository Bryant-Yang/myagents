...
时不会强制跳回底部。输入 `@` 会列出 daemon 当前注册的 Agent，可用项优先并标出
未就绪项，支持方向键、Enter/Tab 和连续多选。活动抽屉保留当前与最近任务；“详情”按 command 展开阶段、
工具、权限、控制和输出统计，daemon 重启后也能从历史消息重新打开最近详情，
没有持久终态的旧任务会明确显示为“上次运行中断”。
运行中的任务可以取消或尝试既有
capability-bounded 插话。
