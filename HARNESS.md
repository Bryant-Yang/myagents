...
- DSH 只启动 stock `dsh --profile myagents`；普通与 `workspace_write` 轮固定
  ...
  `@myagents/dsh-acp-host`，以及解析后的 host 版本、
  `dsh.bundle.patch=./cordis.patch.yml`、entry/patch 与 runtime 版本均精确匹配
  checked-in `dsh_acp/plugin/runtime-contract.json`，缺一即
...
- 工具生命周期按 `(command_id, agent, tool_call_id)` 合并；协议缺少 ID 时
  ...
  默认只显示终态、当前阶段与工具汇总；非浏览态最多投影三张未结束卡和最近
  一张终态卡，运行/取消中优先于排队卡，其余只显示收起计数。`Ctrl+G` 才进入
  完整活动列表并恢复较高的滚动视口；最近终态按实际 terminal 迁移顺序而非
  command 创建顺序确定，活动区任何失焦都恢复紧凑投影。活动卡逐张维护
  展开态，`Ctrl+G` 进入
