...
describe('automation-only ACP bridge', () => {
  ...
  it('rejects a non-function runtime setup before touching host services', () => {
    expect(() => {
      AcpPlugin.apply({} as Context, {
        provider: 'mock',
        model: 'mock',
        setup: 'not-a-function' as never,
  ...
  it('advertises fresh text sessions with direct close even without durable load services', async () => {
  ...
  it('does not advertise load when persistence lacks the public list and locate proof seam', async () => {
    harness = await makeBridgeHarness({
      config: { sessionsRoot: process.cwd() },
      beforeAcp(ctx) {
        ctx.provide('sessionQuery', { readSession: () => Promise.resolve(undefined) } as never)
    ...
    expect(response.agentCapabilities?.loadSession).not.toBe(true)
    await expect(harness.client.loadSession({
      sessionId: 'unproved',
      cwd: process.cwd(),
      mcpServers: [],
    })).rejects.toThrow(/Method not found|session\/load/i)
  })
  ...
  it('rejects an absent product route before touching host services', () => {
    expect(() => {
      AcpPlugin.apply({} as Context, {
        provider: undefined,
        model: undefined,
    ...
    }).toThrow('provider and model must be non-empty')
  })
  ...
  it('requires the exact canonical host workspace and no MCP servers', async () => {
    harness = await makeBridgeHarness()
    await harness.client.initialize({ protocolVersion: PROTOCOL_VERSION, clientCapabilities: {} })

    await expect(harness.client.newSession({ cwd: 'relative', mcpServers: [] }))
      .rejects.toThrow(/exactly equal the canonical host workspace/)
    await expect(harness.client.newSession({ cwd: '/tmp', mcpServers: [] }))
      .rejects.toThrow(/exactly equal the canonical host workspace/)
