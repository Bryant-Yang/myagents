...
describe('ACP prompt lifecycle', () => {
  ...
  it('maps a max-token turn truthfully without losing its committed text', async () => {
  ...
  it('fails the prompt boundary for a merge-extended turn ending', async () => {
    harness = await makeBridgeHarness({ script: [textResponse('committed before unknown ending')] })
    const sessionId = await newSession(harness)
    const agent = harness.ctx.agents.get(SessionId(sessionId))!
    const append = agent.session.append.bind(agent.session)
    vi.spyOn(agent.session, 'append').mockImplementation(((type: string, data: unknown, ...rest: never[]) => {
      if (type === 'turn/end') {
        const end = data as { turn: number }
        return (append as (...args: never[]) => unknown)(type as never, {
          ...end,
          reason: { kind: 'acp-test-extension' } as TurnEndReason,
        } as never, ...rest)
      }
      return (append as (...args: never[]) => unknown)(type as never, data as never, ...rest)
    }) as never)

    await expect(harness.client.prompt({ sessionId, prompt: [{ type: 'text', text: 'go' }] }))
      .rejects.toThrow(/prompt settlement failed: unsupported ACP turn ending: acp-test-extension/)
    vi.restoreAllMocks()
  })
  ...
  it('preserves a completed turn when cancellation races its ordered output drain', async () => {
    harness = await makeBridgeHarness({ script: [textResponse('committed answer')] })
  ...
  it('rejects a correlated agent error when the durable turn ending cannot be appended', async () => {
    harness = await makeBridgeHarness({ script: [textResponse('committed before close failure')] })
    const sessionId = await newSession(harness)
    const agent = harness.ctx.agents.get(SessionId(sessionId))!
    const append = agent.session.append.bind(agent.session)
    vi.spyOn(agent.session, 'append').mockImplementation(((type: string, ...rest: never[]) => {
      if (type === 'turn/end') throw new Error('turn ending append failed')
      return (append as (...args: never[]) => unknown)(type as never, ...rest)
    }) as never)

    await expect(harness.client.prompt({ sessionId, prompt: [{ type: 'text', text: 'go' }] }))
      .rejects.toThrow(/turn failed: turn ending append failed/)
    vi.restoreAllMocks()
  })
  ...
  it('does not borrow an autonomous turn ending for a queued client prompt', async () => {
    ...
    expect(settled).toBe(false)
    await harness.client.cancel({ sessionId })
    await expect(prompt).rejects.toThrow(/queued prompt settled without a turn ending/)
  })
  ...
  it('reports a hook-cancelled turn as cancelled', async () => {
    harness = await makeBridgeHarness({ script: ['hang'] })
    const sessionId = await newSession(harness)
    const prompt = harness.client.prompt({ sessionId, prompt: [{ type: 'text', text: 'go' }] })
    const agent = harness.ctx.agents.get(SessionId(sessionId))!
    await vi.waitFor(() => { expect(agent.status).toBe('running') })
    // ACP's terminal reason describes the turn outcome, regardless of which
    // bounded owner requested the cancellation.
  ...
  it('fails a queued prompt that disappears without a durable turn ending', async () => {
    harness = await makeBridgeHarness({ script: [] })
    const sessionId = await newSession(harness)
    ...
    await expect(harness.client.prompt({ sessionId, prompt: [{ type: 'text', text: 'go' }] }))
      .rejects.toThrow(/queued prompt settled without a turn ending/)
    dispose()
  })
