...
describe('ACP connection ownership', () => {
  ...
  it('session/close drains that session descendants before releasing its owner', async () => {
    harness = await makeBridgeHarness()
    const order: string[] = []
    let drainedParents: readonly Agent[] = []
    harness.ctx.provide('subagents', {
  ...
  it('retains a failed close owner and permits an explicit close retry', async () => {
    harness = await makeBridgeHarness()
    ...
    await expect(harness.client.closeSession({ sessionId })).rejects.toThrow(/transient close failure/)
  ...
  it('forgets a stale close record after the memoized owner teardown retires its registry identity', async () => {
    harness = await makeBridgeHarness()
    await harness.client.initialize({ protocolVersion: PROTOCOL_VERSION, clientCapabilities: {} })
    const { sessionId } = await harness.client.newSession({ cwd: process.cwd(), mcpServers: [] })
    const agent = harness.ctx.agents.get(SessionId(sessionId))!
    const concrete = agent as Agent & { scope: { dispose(): Promise<void> } }
    vi.spyOn(concrete.scope, 'dispose').mockRejectedValueOnce(new Error('scoped teardown failed after retirement'))

    await expect(harness.client.closeSession({ sessionId }))
      .rejects.toThrow(/session close failed: scoped teardown failed after retirement/)
    expect(harness.ctx.agents.get(SessionId(sessionId))).toBeUndefined()
